...
Start with [installation](installation.md). This reference explains standalone
builds, alternative backends, ownership checks and recovery behavior.
...
Use `oncotracer system --path /path/to/project` before
...
The [developer guide](developer_guide.md#build-the-deterministic-copied-executable)
explains how to build a standalone command from an exact source commit.
