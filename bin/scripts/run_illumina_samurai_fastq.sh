...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --build-pon|--pon-normal-samples|--pon-min-normals|--pon-name|--pon-min-mapq|--pon-r-container)
      echo "ERROR: local sample-derived panel construction was removed; NORMAL rows are analyzed as independent samples" >&2
      exit 2
      ;;
    -h|--help) usage; exit 0 ;;
...
if normals:
    raise SystemExit(
        "ERROR: the frozen Nextflow comparator cannot CNA-call NORMAL rows independently; "
        "use native OncoTracer v2 for mixed or NORMAL-only cohorts"
    )
